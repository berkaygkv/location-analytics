...
def get_epsilon(df_rakip, split=False):
    ...
    -   Ma?azalar?n birbirine olan uzakl?klar?ndan bir distance matrix olu?turulur ve her sat?rdaki minimum de?eri al?n?r. 
    -   Böylece minimum distance'lardan olu?an bir liste elde edilir.
    -   Çok büyük uzakl?kl??a sahip say?lar? listeden ç?karmak amac?yla %60'l?k percentile de?eri threshold olarak belirlenir.
    -   %60'l?k percentile'? sa?layan de?erler üzerinden listenin Mean ve Std hesaplan?r.
    -   Hesaplanan Mean ve Std üzerinden ise linear bir (custom) fonksiyon kurularak epsilon de?eri bulunur.
    -   Epsilon için maksimum 0.75, minimum 0.18 de?erleri makul kabul edilmi?tir.

    """
    df1_filtered = df_rakip.copy().reset_index(drop=True).reset_index()
    df1_filtered['geo'] = df1_filtered.apply(lambda x: f"{x['latitude']},{x['longitude']}", axis=1)
    ...
    arr = np.where((distance_matrix > outlier_threshold) | (distance_matrix == 0), np.nan, distance_matrix)
...
def read_rakip_df(city_name, data_path):
    ...
    use_cols = ['Nokta ID','Kategori 5', 'İl', 'İlçe', 'Mahalle','Latitude - Enlem - Y', 'Longitude - Boylam - X']
    ...
    df_rakip = df_rakip.copy().rename(columns={'Mahalle':'mahalle','Latitude': 'latitude', 'Longitude':'longitude', 'Marka':'isim', 'İl':'il', 'İlçe':'ilce'})
...
def filter_mahalle(city_name, data_path, raw_data_directory):
    ...
    df_area = gpd.GeoDataFrame(_df, geometry='geometry').drop(columns=['polygons', 'url', 'mahalle', 'status', 'il', 'ilce']).rename(columns={'mahalle_key': 'key'})
    return df_area

def read_and_convert_mahalle_polygon(raw_data_directory):
    _df = pd.read_excel(raw_data_directory + 'TR_mahalle_geodf.xlsx').dropna(subset=['geometry']).query('status == "OK"')
    _df = _df.applymap(lambda x: x.strip() if isinstance(x, str) else x)
    _df['polygons'] = _df['polygons'].map(listing)
    _df['geometry'] = _df.apply(lambda x: Polygon(x['polygons']) if isinstance(x['polygons'], list) else np.nan, axis=1)
    return _df
...
def excel_saving(sheet_name, output_data_directory, raw_data_directory, city_name, wb_1, wb_2, latest_row):
    ...
    for (row, col), source_cell  in ws_1._cells.items():
        if row == 1 or row == sample_size or row == sample_size - 1:
            ...
            if row == latest_row or (row == latest_row - 1 and cell.column_letter == 'X'):
        ...
        if col != prev_col:
            ws_2.column_dimensions[cell.column_letter].width = ws_1.column_dimensions[cell.column_letter].width
        prev_col = col
        
    first = FormatObject(type='percent', val=0)
    second = FormatObject(type='percent', val=33)
    third = FormatObject(type='percent', val=67)
    iconset = IconSet(iconSet='3Arrows', cfvo=[first, second, third], showValue=None, percent=True, reverse=None)
    ...
    excel_path = output_data_directory + f'{city_name}/{city_name} ' + 'Yayilim Modellemesi.xlsx'
    print(excel_path)
