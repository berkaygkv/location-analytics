...
def convert_matrix_data(response):
    ...
    df_matrix = df_matrix.query('keys == "city_name" or keys == "county_name" or keys == "district_name" or keys == "distance"')
    ...
    df_concat = pd.concat([distance[['id', 'val']], city_name[['val']], county_name[['val']], district[['val']]], axis=1)
    df_concat.columns = ['id', 'distance', 'city_name', 'county_name','district']
    df_concat['key'] = df_concat['city_name'] + ' - ' + df_concat['county_name'] + ' - ' + df_concat['district'] 
    df_concat.drop(columns=['city_name','county_name', 'district'], inplace=True)
    df_unstack = df_concat.reset_index(drop=True).set_index(['id', 'key'], append=True).unstack(level=-1)['distance']
    ...
    df_distance_matrix = df_distance_matrix.applymap(lambda x: np.nan if not len(x) else x[0])
...
def extract_closest_districts(df_distance_matrix):
    ...
    df_matrix_melt = df_distance_matrix.melt(id_vars=['isim'], value_vars=df_distance_matrix.columns.drop('isim')).dropna(subset=['value']).sort_values(['isim','value'])
    df_matrix_groupby = df_matrix_melt.groupby('isim').agg({'variable':[lambda x: list(x)[:5], 'count']}).droplevel(0, axis=1).rename(columns={'<lambda_0>': 'list_of_districts', 'count':'count_of_districts'})
    df_matrix_explode = df_matrix_groupby.explode('list_of_districts').reset_index()
    df_matrix_explode['order'] = 1
    df_matrix_explode['order'] = df_matrix_explode.groupby('isim').order.transform('cumsum')
    df_matrix_explode['order'] = df_matrix_explode['order'].map(lambda x: f"En yakin {x}. Mahalle")
    df_matrix_pivot = df_matrix_explode.pivot(index='isim', columns='order', values='list_of_districts')
